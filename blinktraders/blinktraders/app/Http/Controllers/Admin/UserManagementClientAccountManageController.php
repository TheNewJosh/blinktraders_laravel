...
class UserManagementClientAccountManageController extends Controller
{
    ...
    public function kyc(Request $request)
    {
        ...
        if($request->kyc_verify == 1){
            Mail::to($user_id->email)->send(new KycAproved($user_id));
        }
        if($request->kyc_verify == 0){
            Mail::to($user_id->email)->send(new KycRejected($user_id));
        }
